...
*/
func GenerateDao(orgTableName string, appId, tableName, daoDir, modelDir, idType, baseDaoPackage string) {
	...
	if len(modelDir) > 1 && modelDir[0:2] == "./" {
		modelDir = modelDir[1:]
	}
	genCreateFun(f, appId, tableName, idType, modelDir)
	genGetByIdFun(f, appId, tableName, idType, modelDir)

	_ = os.MkdirAll(daoDir, os.ModePerm)
	fileName := daoDir + "/" + inflection.Singular(tableName) + ".dao.go"
...
*/
func genGetByIdFun(f *jen.File, appId, tableName, idType, modelDir string) {
	...
	f.Comment("通过ID获取").Line().Func().Params(
		...
		jen.Id(entityVarName).Qual(appId + modelDir, entityName),
		jen.Id("err").Id("error"),
	).Block(
...
*/
func genCreateFun(f *jen.File, appId, tableName, idType, modelDir string) {
	entityName := helper.SnakeCase2CamelCase(inflection.Singular(tableName), true)
	entityDaoName := entityName + "Dao"
	...
	f.Comment("创建").Line().Func().Params(
		...
		jen.Id("entity").Id("*").Qual(appId + modelDir, entityName),
	).Params(
		jen.Id("error"),
	).Block(
